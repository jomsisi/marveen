...
describe('renderHeartbeatClaudeMd', () => {
  ...
  it('is fully driven by the identity -- distinct configs render distinctly', () => {
    ...
    // Mixed on purpose: after the merge the generator emits the relay call and
    // the schedules URL (our side) AND the store-dir env plus the metrics script
    // (upstream's). Measured against the merged template, not chosen by side --
    // the three assertions dropped here ("to":"atlas", claudeclaw.db,
    // /api/messages) match nothing the generator produces any more.
